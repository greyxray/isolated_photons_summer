...
void Hist::PlotCrossSec() {
  ...
  leg_1st2nd[2] = new TLegend(0.23, 0.289, 0.63, 0.588);
  ...
  for(Int_t i=0; i<n_cross; i++) {
    ...
    leg_1st2nd[i]->AddEntry(h_1st_cross[i][0], "Matthew", "p");
    leg_1st2nd[i]->AddEntry(h_cross[i][0], "Oleg & Nataliia", "p");
  }
  ...
  for(Int_t i=0; i<n_cross; i++){
    ...
    c_cross[i]->Print("comp_Matt_" + s_var[i] + ".eps");
    c_cross[i]->Print("comp_Matt_" + s_var[i] + ".C");
    c_cross[i]->Print("comp_Matt_" + s_var[i] + ".png");
  }
