...
Hist::Hist()
{
  ...
  s_var[4] = "et_jet";
  s_var[5] = "eta_jet";
  lumi_data[0] = 134.15997;
  lumi_data[1] = 54.79574;
  lumi_data[2] = 142.93778;
  ...
  */
  ...
  f_unity = new TF1("Unity", "0", -100., 500.);
  f_unity->SetLineWidth(1);
  f_unity->SetLineColor(32);
  for(Int_t i=0; i<n_periods; i++) {
    TString s; s.Form("/data/zenith234b/kuprash/analyses/prompt_photons/data" + s_periods[i] + "_inclusive_prph_Matt_trig.root");
    file_data[i] = new TFile(s, "READ");
    s.Form("/data/zenith234b/kuprash/analyses/prompt_photons/mc_bg_norad" + s_periods[i] + "_inclusive_prph_Matt_trig.root");
    file_norad[i] = new TFile(s, "READ");
    s.Form("/data/zenith234b/kuprash/analyses/prompt_photons/mc_bg_rad" + s_periods[i] + "_inclusive_prph_Matt_trig.root");
    file_rad[i] = new TFile(s, "READ");
    if(s_periods[i]!="0607p") {
      s.Form("/data/zenith234b/kuprash/analyses/prompt_photons/mc_prph" + s_periods[i] + "_inclusive_prph_Matt_trig.root");
  ...
  for(Int_t i=0; i<n_periods; i++) {
    ...
    for(Int_t j=0; j<number_etbins; j++) {
      TString s; s.Form("h_deltaz_et_%i", j);
      cout << "reading " << s << " ..." << endl;
  ...
  for(Int_t i=0; i<n_periods; i++) {
    for(Int_t j=0; j<number_etbins; j++) {
      if(i==0) {
	TString s; s.Form("h_deltaz_et_data_sum_%i", j);
	...
	s.Form("h_deltaz_et_prph_sum_%i", j);
  ...
  for(Int_t i=0; i<number_etbins; i++) {
    h_deltaz_et_res[i] = (TH1D*)h_deltaz_et_rad_sum[i]->Clone();
...
void Hist::PlotAcceptance() {
  ...
  for(Int_t i=0; i<n_cross; i++)
    {
      h_acceptance_rad[i] = (TH1D*)h_det_rad_sum[i]->Clone();
      //     h_acceptance_rad[i]->Sumw2();
      ...
      for(Int_t j=0; j<h_acceptance_rad[i]->GetNbinsX(); j++)
	{
	  cout << "bin " << j << ": " << h_det_rad_sum[i]->GetBinContent(j+1) << " "
	       << h_had_rad_sum[i]->GetBinContent(j+1) << endl;
	  //	 cout << h_acceptance_rad[i] << GetBinContent(j+1) << endl;
	}

      h_acceptance_prph[i] = (TH1D*)h_det_prph_sum[i]->Clone();
      //     h_acceptance_prph[i]->Sumw2();
  ...
  leg_acc->AddEntry(h_acceptance_rad[0], "LL photons", "p");
  ...
  for(Int_t i=0; i<n_cross; i++)
    {
      ...
      cout << "before shift: " << h_acceptance_prph[i]->GetXaxis()->GetXmin() << " " << h_acceptance_prph[i]->GetXaxis()->GetXmax() << " first bin center: " << h_acceptance_prph[i]->GetBinCenter(1) <<endl;
      Double_t xmin = h_acceptance_prph[i]->GetXaxis()->GetXmin();
      Double_t xmax = h_acceptance_prph[i]->GetXaxis()->GetXmax();
      ...
      cout << "after shift: " << h_acceptance_prph[i]->GetXaxis()->GetXmin() << " " << h_acceptance_prph[i]->GetXaxis()->GetXmax() << " first bin center: " << h_acceptance_prph[i]->GetBinCenter(1)<< endl;
      h_acceptance_prph[i]->DrawClone("P E X0 SAME");
      cout << "after drawing: " << h_acceptance_prph[i]->GetXaxis()->GetXmin() << " " << h_acceptance_prph[i]->GetXaxis()->GetXmax() << " first bin center: " << h_acceptance_prph[i]->GetBinCenter(1)<< endl;
      if(i==3) leg_acc->Draw();
    }
...
void Hist::PlotFitInBinsOfCrossSec() {
  ...
  for(Int_t i=0; i<number_etbins; i++)
    {
      TString s; s.Form("h_window_fit_et_%i", i); 
      h_window_fit_et[i] = new TH2D(s, "", 10, 0., 1.5, 10, 0., ymax_et[i]);
    }

  TPaveText *pave_et[number_etbins];
  ...
  for(Int_t i=0; i<number_etbins; i++) {
    ...
    cout << "dz before shift: " << h_deltaz_et_data_sum[i]->GetXaxis()->GetXmin() << " " << h_deltaz_et_data_sum[i]->GetXaxis()->GetXmax() << " first bin center: " << h_deltaz_et_data_sum[i]->GetBinCenter(1) <<endl;
    ...
    cout << "dz after shift: " << h_deltaz_et_data_sum[i]->GetXaxis()->GetXmin() << " " << h_deltaz_et_data_sum[i]->GetXaxis()->GetXmax() << " first bin center: " << h_deltaz_et_data_sum[i]->GetBinCenter(1) <<endl;
    h_deltaz_et_data_sum[i]->Draw("P E X0 SAME");
  ...
  for(Int_t i=0; i<number_etabins; i++)
    {
      TString s; s.Form("h_window_fit_eta_%i", i); 
