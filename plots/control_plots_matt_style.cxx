...
int main(int argc, char *argv[])
{
  ...
  set_hist_atributes(2,  "h_phjet_cal_empz", "Photon_with_jet", "cal E-P_{z} (GeV)", 30, 70, 0.1, kFALSE, 10);
  ...
  set_hist_atributes(6,  "h_prph_energy", "Photon_with_jet", "E_{T, photon} (GeV)", 2, 16, 0.1, kFALSE, 10);
  set_hist_atributes(7, "h_prph_eta", "Photon_with_jet", "#eta_{photon}", -0.8, 1, 0.1, kFALSE, 10);
  set_hist_atributes(8, "h_prph_phi", "Photon_with_jet", "#phi_{photon}", -3.5, 3.5, 0.1, kFALSE, 10);
  set_hist_atributes(9,  "h_comp_Q2", "Photon_with_jet", "Q^{2} (GeV^{2})", 0., 400., 0.1, kTRUE, 10);
  set_hist_atributes(10,  "h_prph_cell_energy_frac", "Photon_with_jet", "EMC energy fraction", 0.89, 1.01, 0.1, kTRUE, 10);
  set_hist_atributes(11, "h_prph_jet_energy_frac", "Photon_with_jet", "jet energy fraction", 0.89, 1.01, 0.1, kFALSE, 10);
  ...
  //  set_hist_atributes(9,  "h_prph_energy", "Photon_with_jet", "E_{T, photon} (GeV)", 2, 16, 0.1, kFALSE, 10);
  //  set_hist_atributes(10, "h_prph_eta", "Photon_with_jet", "#eta_{photon}", -0.8, 1, 0.1, kFALSE, 10);
  //  set_hist_atributes(11, "h_prph_phi", "Photon_with_jet", "#phi_{photon}", -3.5, 3.5, 0.1, kFALSE, 10);

  set_hist_atributes(12, "h_comp_jet_e", "Photon_with_jet", "E_{T, jet} (GeV)", 0, 45, 0.1, kTRUE, 20);
  set_hist_atributes(13, "h_phjet_jet_eta", "Photon_with_jet", "#eta_{jet}", -1.6, 1.9, 0.1, kFALSE, 10);
  set_hist_atributes(14, "h_phjet_jet_phi", "Photon_with_jet", "#phi_{jet}", -3.5, 3.5, 0.1, kFALSE, 10);

  set_hist_atributes(15, "h_et_jet_photon_ratio", "Photon_with_jet", "E_{T, jet} / E_{T, photon}", 0., 8., 0.1, kTRUE, 5);
  set_hist_atributes(16,  "h_prph_cell_energy_frac", "Photon_with_jet", "EMC energy fraction", 0.89, 1.01, 0.1, kTRUE, 10);
  set_hist_atributes(17, "h_prph_jet_energy_frac", "Photon_with_jet", "jet energy fraction", 0.89, 1.01, 0.1, kTRUE, 10);

  set_hist_atributes(18, "h_phjet_dphi", "Photon_with_jet", "#phi_{jet} - #phi_{photon}", 0., 3.4, 0.1, kFALSE, 10);
  set_hist_atributes(19, "h_et_3system_event_ratio", "Photon_with_jet", "(E_{T, #gamma} + E_{T, jet} + E_{T, electron}) / E_{T, event}", 0., 1., 0.1, kFALSE, 10);
  set_hist_atributes(20,  "h_phjet_electron_probability", "Photon_with_jet", "Sinistra Probability", 0.89, 1, 0.1, kTRUE, 10);
  ...
  */
  ...
  for(Int_t i=0; i<n_hist; i++)
    {
      if(i!= 23) {
      cout << " cd " << i/6 << " " << i%6+1 << endl;
      c1[i/6]->GetPad(i%6+1)->cd();
      c1[i/6]->GetPad(i%6+1)->SetLogy(hist_logy[i]);
      ...
      c1[i/6]->GetPad(i%6+1)->RedrawAxis();
