...
int main(int argc, char *argv[])
{
	...
	//set_hist_atributes(11, "h_x_gamma", "Photon_with_jet", "x_{#gamma}", 0, 1.2, 0.1, kFALSE, 10);
	set_hist_atributes(57,  "h_x_gamma", "Photon_with_jet", "h_x_gamma", 0., 1.0, 0.0, kFALSE, 1);
	set_hist_atributes(58,  "h_phjet_deta", "Photon_with_jet", "h_phjet_deta", -2.2, 2.7, 0.0, kFALSE, 1);
	set_hist_atributes(59,  "h_phjet_dphi", "Photon_with_jet", "h_phjet_dphi", 0.0, 180., 0.0, kFALSE, 1);
	set_hist_atributes(60,  "h_x_pomeron", "Photon_with_jet", "h_x_pomeron", 0., 0.03, 0.0, kFALSE, 1);
	...
	//  TString s_process = "_rew_etaph_eph_bg.root";
	//  TString s_process = "_norew.root";  
	TString s_process = "_test_zero.root"; //"_q2etapheph_rew_n.root";
	//  TString s_process1 = "_rew_eph_bg.root"; "_rew_q2_ybbb_eph.root"; "_rew_q2_y_2nd.root";
	...
	for(Int_t p = 0; p < n_periods; p++) 
	{
		...
		for(Int_t i = 0; i < n_hist; i++)
		{
			...
			{
				file[p][3]->cd();
				cout << "reading mc_prph "  << i << " ..." << endl << s_path + "mc_prph" + s_period[p] + s_process << endl;
	...
	//***********************
	//    CREATING HISTOGRAMS FOR SUMMING OVER PERIODS
	//***********************
	for(Int_t i = 0; i < n_hist; i++)
	{      
		if(g_include_04p_data)//false 
		{
			file_data04p->cd();
		...
		hist_mc_rad_sum[i]->SetLineStyle(2);
		...
		hist_mc_norad_sum[i]->SetLineStyle(3);
	...
	//***********************
	//    END CREATING HISTOGRAMS FOR SUMMING OVER PERIODS
	//***********************

	//***********************
	//    SCALE TO LUMINOSITY //scale first period monte carlo to data luminosity
	//***********************
	...
	for(Int_t i = 0; i < n_periods; i++) //3
		cout << s_period[i] << ", data: " << hist_data[i][0]->Integral() << ", rad mc: " << hist_mc_rad[i][0]->Integral() << endl << "lumi data: " << lumi_data[i] << ", lumi mc: " << lumi_mc_bg[i] << endl;

	for(Int_t  i = 0; i < n_hist; i++) //57
	{
		hist_mc_rad_sum[i]   -> Scale(lumi_data[0] / lumi_mc_bg[0]);
		hist_mc_norad_sum[i] -> Scale(lumi_data[0] / lumi_mc_bg[0]);
	}
	cout << "AFTER LUMI BG SCALING 0405E data: " << hist_data_sum[0]->Integral() << ", rad mc: " << hist_mc_rad_sum[0]->Integral() << endl;
	//***********************
	//    END SCALE TO LUMINOSITY //scale first period monte carlo to data luminosity
	//***********************

	//***********************
	//    SUM HISTOGR FOR ALL PERIODS //sum histogramms for all periods
	//***********************
	for(Int_t i = 0; i < n_hist; i++) //57
	{
		for(Int_t p = 1; p < n_periods; p++) //3
		{
	...
	cerr << "ADDING : OK" << endl;
	//***********************
	//    END SUM HISTOGR FOR ALL PERIODS //sum histogramms for all periods
	//***********************
	...
	cout << " AFTER STRANGE SCALING hist_mc_sum[1]: " <<  hist_mc_sum[1]->Integral() << ", hist_mc_norad_sum: " << hist_mc_norad_sum[1]->Integral() << endl;
	//***********************
	//    END STRANGE SCALING
	//***********************  
	...
	//    FITTING BB
	//***********************
	TFitter* minimizer = new TFitter(1);//as minuit TFitter(Int_t maxpar) 
	...
	//***********************
	//    END FITTING Gauss chi2 function
	//***********************
	...
	//***********************
	// END calculate chi2 as function of fit parameter (prph fraction)
	//***********************
	...
	for(Int_t i=0; i<n_hist; i++)
	{
		scale_mc = scale_mc_norad = scale_mc_rad = 1.;
		...
		hist_res[i]->Add(hist_mc_sum[i], 1.);//+QQ
		hist_res[i]->Add(hist_mc_norad_sum[i], 1.);//+bg
	...
	cout << "After fitting: " << endl << "data: " << endl;
	for(Int_t i=0; i<hist_data_sum[1]->GetNbinsX(); i++)
		cout << i << "   " << hist_data_sum[1]->GetBinContent(i+1) << "   " << hist_data_sum[1]->GetBinError(i+1) << endl;
	...
	Double_t hist_mc_sum_integral = 0;// MC SIGNAL
	...
	//MC LL
	Double_t hist_mc_rad_sum_integral = 0;// MC LL
	...
	/*for(Int_t i=0; i < number_of_pages; i++)// sets  plot_style_utils.h
	  {
	  c1[i]->Divide(3, 2);
	  make_clean_pads(c1[i], 6, 0, 0);    /*make_clean_pads(TCanvas *c, Int_t n, Int_t gridx, Int_t gridy)* /
	  }*/
	...
	for(Int_t i = 0; i < n_hist; i++)
	{
		if(i != 23) //i != 23 phjet_electron_probability //set_hist_atributes(23,  "h_phjet_electron_probability", "Photon_with_jet", "Sinistra Probability", 0.89, 1, 0.1, kTRUE, 10);
		{
			...
			hist_data_sum[i]->DrawClone("P E X0 SAME");  //option = "P" draw the bins as markers."E"	Draw error bars."X0"	When used with one of the "E" option, it suppress the error bar along X as gStyle->SetErrorX(0) would do.
