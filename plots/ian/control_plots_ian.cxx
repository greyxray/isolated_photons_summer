...
//!-!      hist_mc_norad_sum[i]->Scale(lumi_data[0]/lumi_mc_bg[0]);
  ...
  for(Int_t i=0; i<n_hist; i++)
      ...
      for(Int_t p=1; p<n_periods; p++)
	{
	  ...
	  cerr << "data - ok" << endl;
	  hist_mc_rad_sum[i]->Add(hist_mc_rad[p][i], 1./*!-!lumi_data[p]/lumi_mc_bg[p]*/);
	  cerr << hist_mc_rad[p][i]->GetNbinsX() << endl;
	  cerr << "mc_rad - ok" << endl;
	  hist_mc_norad_sum[i]->Add(hist_mc_norad[p][i], 1./*!-!lumi_data[p]/lumi_mc_bg[p]*/);
...
//!-!      hist_mc_norad_sum[i]->Scale((hist_data_sum[i]->Integral()-hist_mc_rad_sum[i]->Integral())/hist_mc_norad_sum[i]->Integral());
  ...
  TF1* f_gaus = new TF1("fit_gaus", "gaus");
  hist_check_fit_error->Fit(f_gaus, "QI", "", 0., 1.);
  cout << "Multiple fit : " << endl;
  for(Int_t i=0; i<3; i++) {
    cout << f_gaus->GetParameter(i) << " +- ";
    cout << f_gaus->GetParError(i) << endl;
  }
  ...
  c_fit_err->Print("fit_err_ian.eps");
