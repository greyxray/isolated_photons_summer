 ...
 */ 
  ...
  const Int_t n_binning = 9;
  //  Int_t rebin_binning[n_binning] = {1, 2, 4, 5, 8, 10, 20, 25, 40};
  Int_t rebin_binning[n_binning] = {1, 2, 4, 5, 6, 10, 15, 25, 30};
  Double_t x_binning[n_binning];
  Double_t y_binning[n_binning];
  Double_t y_binning_Gauss[n_binning];
  Double_t y_err_binning[n_binning];
  Double_t y_err_binning_Gauss[n_binning];
  Double_t x_err_binning[n_binning] = {0.};

  Double_t y_par_binning[n_binning];
  Double_t y_par_binning_Gauss[n_binning];
  Double_t y_par_err_binning[n_binning];
  Double_t y_par_err_binning_Gauss[n_binning];

  for(Int_t i=0; i<n_binning; i++) {
    //    Int_t nbins = 
    x_binning[i] = float(300) / float(rebin_binning[i]);
    ...
    hist_data_sum[0]->Rebin(rebin_binning[i]);
    hist_mc_sum[0]->Rebin(rebin_binning[i]);
    hist_mc_rad_sum[0]->Rebin(rebin_binning[i]);
    hist_mc_norad_sum[0]->Rebin(rebin_binning[i]);
    //    hist_ratio[0]->Rebin(2);

    hist_data_sum[1]->Rebin(rebin_binning[i]);
    hist_mc_sum[1]->Rebin(rebin_binning[i]);
    hist_mc_rad_sum[1]->Rebin(rebin_binning[i]);
    hist_mc_norad_sum[1]->Rebin(rebin_binning[i]);
    //    hist_ratio[1]->Rebin(2);    
    ...
    y_binning[i] = chi_squared;//param[0];
    y_par_binning[i] = param[0];
    y_err_binning[i] = 0.;//param_err[0];
    y_par_err_binning[i] = param_err[0];
    ...
    y_binning_Gauss[i] = chi_squared_Gauss;//param_Gauss[0];
    y_err_binning_Gauss[i] = 0.;//param_err_Gauss[0];
    y_par_binning_Gauss[i] = param_Gauss[0];
    y_par_err_binning_Gauss[i] = param_err_Gauss[0];
  }
  //
  // chi2
  //
  TGraphErrors *tge = new TGraphErrors(n_binning, x_binning, y_binning, x_err_binning, y_err_binning);
  TGraphErrors *tge_Gauss = new TGraphErrors(n_binning, x_binning, y_binning_Gauss, x_err_binning, y_err_binning_Gauss);
  ...
  tge_Gauss->SetLineColor(kBlue);
  TLegend *leg_binning = new TLegend(0.4, 0.75, 0.9, 0.9);
  leg_binning->SetFillColor(0);
  leg_binning->SetBorderSize(0);
  leg_binning->AddEntry(tge, "Barlow-Beeston fit", "lp");
  leg_binning->AddEntry(tge_Gauss, "Gauss approach", "lp");
  ...
  sign_window(cc->GetPad(1), h_window_binning, "No. of bins", "#chi^{2}", "");
  cc->Draw();
  cc->GetPad(1)->cd();
  h_window_binning->DrawClone();
  tge->DrawClone("SAME LP");
  tge_Gauss->DrawClone("SAME LP");
  leg_binning->DrawClone();
  cc->Print("chi2.eps");
  cc->Print("chi2.C");
  //
  // fit parameter
  //
  TGraphErrors *tge_par = new TGraphErrors(n_binning, x_binning, y_par_binning, x_err_binning, y_par_err_binning);
  TGraphErrors *tge_par_Gauss = new TGraphErrors(n_binning, x_binning, y_par_binning_Gauss, x_err_binning, y_par_err_binning_Gauss);
  ...
  sign_window(cc1->GetPad(1), h_window_binning, "No. of bins", "fit parameter", "");
  cc1->Draw();
  cc1->GetPad(1)->cd();
  //  h_window_binning->GetYaxis()->SetRangeUser(0., 0.6);
  h_window_binning->GetYaxis()->SetRangeUser(0., 1.);
  h_window_binning->DrawClone();
...
Double_t chi2(double* par, Int_t& ndf, Double_t& left, Double_t& right, TString s_method)
{
  ...
  for(Int_t i=hist_data_sum[index]->FindBin(left)-1; i<hist_data_sum[index]->FindBin(right)/*GetNbinsX()*/; i++)
    {
