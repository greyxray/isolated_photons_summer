...
int main(int argc, char *argv[])
{
	...
	set_hist_atributes(61,  "h_phjet_deta_unmarged", "Photon_with_jet", "#Delta#eta", -2.2, 2.7, 0.0, kFALSE, 10);
	...
	set_hist_atributes(65,  "h_x_gamma_unmarged", "Photon_with_jet", "x_{#gamma}, rebin, per bin", 0., 1.0, 0.0, kFALSE, 10);

	//22
	set_hist_atributes(66,  "h_x_pomeron", "Photon_with_jet", "x_{#p}, per bin", 0., 0.03, 0.0, kFALSE, 1);
	set_hist_atributes(67,  "h_x_gamma", "Photon_with_jet", "x_{#gamma}, per bin", 0., 1.0, 0.0, kFALSE, 1);
	set_hist_atributes(68,  "h_phjet_deta", "Photon_with_jet", "#Delta#eta, per bin", -2.2, 2.7, 0.0, kFALSE, 1);

	set_hist_atributes(69,  "h_phjet_dphi", "Photon_with_jet", "#Delta#phi, per bin", 0.0, 180., 0.0, kFALSE, 1);

	TString s_period[n_periods] = {"0405e", "06e", "0607p"};
	TFile *file[n_periods][4];
	TFile *file_data04p;

	TString s_path = "my_";//"../my_"
	...
	for(Int_t i = 0; i < n_hist; i++)
	{
		...
		hist_mc_photon_sum[i] -> Add(hist_mc_sum[i]);//LL
	}
	cerr << "ADDING : OK" << endl;
	...
	*/
	...
	for(Int_t i = 0; i < n_hist; i++)
	{
		...
		hist_mc_sum[i]->Scale(param[0]);// Scaled THE VALUE of QQ
...
*/
	//Form RES MC hist_res, scaling fo <>0,35dZ - why so?
	for(Int_t i = 0; i < n_hist; i++)
	{
