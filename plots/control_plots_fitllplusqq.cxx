...
//   set_hist_atributes(56,  "phjet_etph_y_lt_05", "Photon_with_jet", "E_{T}^{#gamma} (GeV) (y < 0.5)", 4., 15., 0.1, kFALSE, 1);
  ...
  TString s_process = "_q2etaph_rew.root";  
                   
  //  TString s_process1 = "_rew_eph_bg.root";
  ...
  for(Int_t i=0; i<n_hist; i++)
	...
	hist_data_sum[i]->Add(hist_data04p[i]);
      ...
      hist_mc_photon_sum[i]->Add(hist_mc_sum[i], (lumi_data[0]+lumi_data[1]+lumi_data[2])/lumi_mc_prph);
    }
  ...
  for(Int_t i=0; i<n_hist; i++)
    {
     ...
     //     hist_mc_norad_sum[i]->Scale((hist_data_sum[i]->Integral()-hist_mc_rad_sum[i]->Integral())/hist_mc_norad_sum[i]->Integral());
      scale_hist_to_hist_with_err(hist_mc_photon_sum[i], hist_data_sum[i]);
      scale_hist_to_hist_with_err(hist_mc_norad_sum[i], hist_data_sum[i]);

      //     hist_mc_norad_sum[i]->Scale(hist_data_sum[i]->Integral()/hist_mc_norad_sum[i]->Integral());
      //     hist_mc_photon_sum[i]->Scale(hist_data_sum[i]->Integral()/hist_mc_photon_sum[i]->Integral());
    } 
  ...
  */
  ...
  for(Int_t i=0; i<n_hist; i++)
    {
      ...
      hist_mc_photon_sum[i]->Scale(param[0]);
      for(Int_t j=0; j<hist_mc_sum[i]->GetNbinsX(); j++)
	{
	  //	  cout << "err before: " << i << " " << j << " " << hist_mc_photon_sum[i]->GetBinError(j+1) << endl;
	  Double_t err = hist_mc_photon_sum[i]->GetBinError(j+1)/param[0];
	  Double_t add_err1 = param_err[0]*hist_mc_photon_sum[i]->GetBinContent(j+1)/param[0];
	  add_err1 *= add_err1;
	  Double_t add_err2 = err*param[0];
	  add_err2 *= add_err2;
	  hist_mc_photon_sum[i]->SetBinError(j+1, TMath::Sqrt(add_err1 + add_err2));
	  //	  cout << "err after: " << i << " " << j << " " << hist_mc_photon_sum[i]->GetBinError(j+1) << endl;
	}

      hist_res[i] = (TH1D*)hist_mc_photon_sum[i]->Clone();
      ...
      for(Int_t j=0; j<hist_mc_norad_sum[i]->GetNbinsX(); j++)
	{
      ...
      hist_res[i]->Add(hist_mc_norad_sum[i]);
    }
  ...
  cout << "There are " << hist_mc_photon_sum[1]->Integral() << " fitted photons" << endl;
  Double_t hist_mc_photon_sum_integral = 0;
  Double_t hist_mc_photon_sum_integral_err = 0;
  for(Int_t i=0; i<hist_mc_photon_sum[1]->GetNbinsX(); i++){
    hist_mc_photon_sum_integral += hist_mc_photon_sum[1]->GetBinContent(i+1);
    hist_mc_photon_sum_integral_err += hist_mc_photon_sum[1]->GetBinError(i+1) * hist_mc_photon_sum[1]->GetBinError(i+1);
  }
  hist_mc_photon_sum_integral_err = TMath::Sqrt(hist_mc_photon_sum_integral_err);
  cout << hist_mc_photon_sum_integral << " +- " << hist_mc_photon_sum_integral_err << endl;
  ...
  leg->AddEntry(hist_mc_photon_sum[0], "LL+QQ", "l");
  ...
  hist_mc_photon_sum[index]->DrawClone("HIST SAME");
  ...
  c_method->Print(fit_method + "_llqq.eps");
  ...
  for(Int_t i=int(left)/*hist_data_sum[index]->FindBin(left)-1*/; i<int(right)/*hist_data_sum[index]->FindBin(right)*//*GetNbinsX()*/; i++)
    {
      ...
      //
      // ====================================================================================================
      diff = hist_data_sum[index]->GetBinContent(i+1) - par[0]*hist_mc_photon_sum[index]->GetBinContent(i+1) - (1-par[0])*hist_mc_norad_sum[index]->GetBinContent(i+1);
      // ====================================================================================================
      //
      // Ian
      //
	...
	//
	// ====================================================================================================
	denom = s_d*s_d + s_l*s_l + par[0]*par[0]*s_photon*s_photon + (1.-par[0])*(1.-par[0])*s_bg*s_bg;
	// ====================================================================================================
	//
	// Ian
	//
