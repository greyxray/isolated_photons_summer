...
Bool_t selector::SelectHadronLevel(Bool_t take_det_event)
{
  ...
  //event selection
    ...
    {
      take_hevent = kFALSE;
      if (check_cuts && nodebugmode) cout << "rejected by cut on true q2 = " << Mc_q2 << endl;
    }
    //electron sel
      ...
      {
        take_hevent = kFALSE;
        if (check_cuts && nodebugmode) cout << "rejected by cut on true electron theta = " << v_true_electron.Theta()*180.0/TMath::Pi() << endl;
      }
      if(Mc_pfsl[3] < 10.)
      {
        take_hevent = kFALSE;
        if (check_cuts && nodebugmode) cout << "rejected by cut on true electron energy = " << Mc_pfsl[3] << endl;
      }
  ...
  //find true photon on gen level
    ...
    if(index_true_photon < 0)
    {
        ...
        if (check_cuts && nodebugmode) cout << "there is no photon in generated level" << endl;
    }
  ...
  //photons cuts
    if(take_hevent && here_is_true_prph) 
    {
      ...
      {
        is_true_prph_candidate = kFALSE;
        if (check_cuts && nodebugmode) cout << "rejected by cut on true photon eta = " << true_photon_eta << endl;
      }

      if (Ktrnjets < 1) 
      {
        is_true_prph_candidate = kFALSE;
        if(check_cuts && nodebugmode) cout << "rejected by Ktrnjets<1: " << Ktrnjets << endl;
      }
      if (index_jet < 0) 
      {
        is_true_prph_candidate = kFALSE;
        if (check_cuts && nodebugmode) cout << "rejected by index_jet<0: " << index_jet << endl;
      }
      if ((Part_p[index_true_photon][3] / Ktrjets[index_jet][3]) < 0.9) 
      {
        is_true_prph_candidate = kFALSE;
        if (check_cuts && nodebugmode) cout << "rejected by photon/jet energy ratio: " << Part_p[index_true_photon][3]/Ktrjets[index_jet][3] << endl;
      }
    }
