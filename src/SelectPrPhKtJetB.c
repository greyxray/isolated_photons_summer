...
Bool_t selector::SelectPrPhKtJetB(Int_t i_true_rad_photon)
{
    ...
    for (Int_t zloop = 0; zloop < Knzufos; zloop++)
    {
        ...
        if(check_cuts)
          cout << "prph candidate " << zloop  << " of " << Knzufos << ", eta = " << zu_eta 
        ...
        if (zu_fmax <= 0.05) 
        {
            take_prph = kFALSE;
            if(check_cuts)
                cout << "photon with eta " << zu_eta << " rejected, because fmax = " << zu_fmax << endl;
        } 
        if (zu_deltaz < 0.0)   
        {
            take_prph = kFALSE;
            if(check_cuts)
                cout << "photon with eta " << zu_eta << " rejected, because deltaz = " << zu_deltaz << endl;
        } 
        ...
        if (zu_eta < -0.7)     
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because eta = " << zu_eta << endl;
        }
        if (zu_eta > 0.9)     
        {
            take_prph = kFALSE;
        ...
        if (zu_et_corr < 4.)  
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because et_corr = " << zu_et_corr << endl;
        }
        if (zu_et_corr > 15.)  
        {
            take_prph = kFALSE;
        ...
        if (Kzufoemcfrac[zloop] < 0.9)
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because emcfrac = " << Kzufoemcfrac[zloop] << endl;
        }
        ...
        if (Kzufotype[zloop] != 31)   
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because zufo type = " << Kzufotype[zloop] << endl;
        }
        // track isolation
        ...
        for (Int_t i = 0; i < Trk_ntracks; i++)
        {
            ...
            if(check_cuts) cout << "track # " << i << " " << tr.Eta() << " " << tr.PseudoRapidity() <<  " " 
        ...
        if (ntrack>0)  
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because ntrack = " << ntrack << endl;
        }
        ...
        if ((Kzufos[zloop][3] * systPhE / zu_jetEnergy) < 0.9)
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "photon with eta " << zu_eta << " rejected, because e_phot/e_jet_cont_phot = " << (Kzufos[zloop][3] * systPhE/zu_jetEnergy)<< endl;
        }
        ...
        if(check_cuts) cout << "cont jet: " << jet_contained_prph.Phi() << " " << jet_contained_prph.Eta() << ", photon: " << zu_phi<< " " << zu_eta  
                         << " dR = " << dR_jet << endl;
        if(dR_jet > 0.2)
        {
            take_prph = kFALSE;
            if(check_cuts)
              cout << "candidate with eta " << zu_eta << " rejected by dR_jet = " << dR_jet << endl;
        }
