		...
		{   
			...
			// Detector level selection
				...
				// finding photon 
					...
					//find photon the same for Data and MC on detector level
				...
				if(take_event)
				{
					...
					//constructing vector on/of zufos without e and photon - input_vec, input_vec_to_zufos
						...
						for(Int_t zloop = 0; zloop < Nzufos; zloop++)
						{
							...
							{
								if(false && check_cuts && !nodebugmode) cout << "rejecting zufo number " << electron_number << " with e = " << Zufo[zloop][3] << " because it is most probably electron " << endl;
								continue;
							}
					...
					// Retrieve the final state jets from KtEvent sorted by Et
						vector<KtLorentzVector> jets 	  = ev.getJetsEt(); // the energy will be corrected
					
					// Retrieve the final state jets from KtEvent sorted by Et
						vector<KtLorentzVector> jets_copy = ev.getJetsEt();	// the energy will be NOT corrected
					...
					if(take_event && here_is_prph)
					{
						for(Int_t jloop = 0; jloop < jets.size(); jloop++)
						{
							if(check_cuts && !nodebugmode) cout << "jet " << jloop << ": calling correction with parameters: " << jets[jloop].eta() 
									<< ", " << jets[jloop].et() << ", " << period << ", " << mc_type << endl;
							
							Double_t et_jet_corr = jet_en_corr_1stana(jets[jloop].eta(), jets[jloop].et(), period, mc_type);
							
							if(check_cuts && !nodebugmode) cout << "jet " << jloop << " of " << jets.size() 
									<< ", eta = " << jets[jloop].eta() 
							...
							{
								take_jet = kFALSE;
								if(check_cuts && !nodebugmode) cout << "jet with eta " << jets[jloop].eta() << " rejected because it is prompt photon jet " << endl;
							}
					...
					if (check_cuts && !nodebugmode) cout << "max_et_accomp_jet_candidate_number: " << max_et_accomp_jet_candidate_number << endl;
						...
						*/
						...
						if(take_event_trig)
						{
							...
							//cout <<"ATTENTION!!!!! FOUND!  entry:"<< entry <<"  Eventnr:"<< Eventnr << endl;
							//exit(1);
