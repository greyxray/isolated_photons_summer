...
Bool_t selector::Process()
{
	...
	*/
	...
	cout << "Number of events to process: " << nentries << "\nq2_cut_low = " << q2_cut_low << "	q2_cut_high = " << q2_cut_high << endl;
	...
	//parameter initialising
		...
		long sel_det_lev(0), sel_had_lev(0);
		phot_count = 0;
		//  check_cuts = kTRUE;
	...
	nodebugmode = true;
	bool printOnlyCuts = false;
	for (Long64_t entry = 0; entry < nentries && debugcontinue; entry++)
	{
		fChain->GetEntry(entry);

		if (entry >= nEvToProcess) break; //debugcontinue = kFALSE;
		// cout << "Ev: " << entry << endl;
		//else if (entry < 2) continue;
		...
		// setting the weights for the photon in MC
		wtx = 1.;
		if (!Data) 
		{
		...
		{   
			...
			// Detector level selection
				...
				//electron selection
					...
					Bool_t take_electron_event = kTRUE,
					       continue_by_q2 = kFALSE;

					if (Sincand < 1)
					{
						take_event = kFALSE;
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Sincand = " << Sincand << endl;
					}
					else if (check_cuts)
					{
					...
					//find electron candidate passing cuts - sinistra_electron_number
						for (Int_t i = 0; i < 1; i++) 
						{
							...
							{
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by det cut q2: "<< Siq2el[i]<< "< q2_cut_low ||  > q2_cut_high"<< endl;
							...
							{
								take_electron_event = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Siecorr[0][2] = " << Siecorr[i][2] * systElE << endl;
							}
							if (Sith[i]*180.0/TMath::Pi() < 139.8 || Sith[i]*180.0/TMath::Pi() > 180.0) //if (Sith[i]*180.0/TMath::Pi() < 139.8 || Sith[i]*180.0/TMath::Pi() > 171.9)
							{
								take_electron_event = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Sith = " << Sith[0] * 180./TMath::Pi() << endl;
							}
							if ((TMath::Abs(Sipos[i][0]) < 14.8) && (Sipos[i][1] > -14.6) && (Sipos[i][1] < 12.5))  
							{
								take_electron_event = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Sipos[0][0] = " << Sipos[0][0] << ", Sipos[0][1] = " << Sipos[0][1] << endl;
							}
						...
						if (!take_electron_event) 
						{
							if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by e investigation, take_electron_event: " << take_electron_event << endl;
							take_event = kFALSE;
						}
				...
				//kinematical cuts	
					...
					{
						take_event = kFALSE;
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Zvtx = " << Zvtx << endl;
					}

					if (Cal_empz < 35. || Cal_empz > 65.) 
					{
						take_event = kFALSE;
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by Cal_empz = " << Cal_empz << endl;
					}
					...
					{
						take_event = kFALSE;
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by nVertTracks = " << nVertTracks << endl;
					}
					...
					if (!spp_trigger) 
					{
						...
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "rejected by trigger condition " << endl;
					}
					if (!take_event_trig) take_event = kFALSE;
					if (check_cuts && !nodebugmode && take_event) cout << "		passed kinematics:" << endl;
				
				//parameter initialising
					//      if (!nodebugmode) cout << "dis sel: " << Runnr << " " << Eventnr << " " << Siq2el[0] << " " << Zvtx << " " << Cal_empz << " " << Sith[0] << " " << Sipos[0][0] << " " << Sipos[0][1] << " " << spp02trigger << endl;
				...
				if (take_event)
				{
					...
					{
						take_event = kFALSE;
						if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "event rejected because was not found zufo that is close to sinistra electron" << endl;
					}
					...
					if (print_interesting_events) 
					{
						...
						if (pz_of_event > 500.)
						{
							...
							if (!nodebugmode) cout << "============ INTERESTING EVENT ================" << endl;
							if (!nodebugmode) cout << "========= " << Runnr << " " << Eventnr << " ================" << endl;
							if (!nodebugmode) cout << " proton **** up: pz_of_event = " << pz_of_event << endl;
							if (!nodebugmode) cout << "===============================================" << endl << endl;      
						}
					}
				}
				...
				// finding photon 
					...
					// Find photon on detector level
					if (use_ktjetb_prph)
					{
						if (check_cuts && !nodebugmode) cout << "send to prpktjetb " << index_true_rad_photon << endl;
						if (SelectPrPhKtJetB(index_true_rad_photon)) here_is_prph = kTRUE; // -->  max_et_candidate_number, max_et_candidate, v_uncorr_prompt_photon, v_corr_prompt_photon, glob_deltaz
					}
					else if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "photon not found on det lev\n";
					// if (use_ktjetb_prph && !here_is_prph) take_event = kFALSE;
				

				
				if (take_event)
				{
					if (check_cuts && !nodebugmode) cout << "max_et_candidate_number: "<< max_et_candidate_number << "\nconstructing vector on/of zufos without e and photon" <<endl;
					//constructing vector on/of zufos without e and photon - input_vec, input_vec_to_zufos
						...
						for (Int_t zloop = 0; zloop < Nzufos; zloop++)
						{
							...
							{
								if (check_cuts && !nodebugmode) cout << "\trejecting zufo number " << electron_number << " with e = " << Zufo[zloop][3] << " because it is most probably electron " << endl;
								continue;
							}
						...
						if (check_cuts && !nodebugmode) cout << Nzufos << " " << input_vec.size() << " hadrons added" << endl;
					...
					//constructing jets
						...
						KtEvent ev(input_vec,type,angle,recom,rparameter); // Construct the KtEvent object 
						vector<KtLorentzVector> jets = ev.getJetsEt(); // Retrieve the final state jets from KtEvent sorted by Et // the energy will be corrected
						vector<KtLorentzVector> jets_copy = ev.getJetsEt();	// Retrieve the final state jets from KtEvent sorted by Et // the energy will be NOT corrected
					
					//counting possible candidates
						Int_t n_jets = 0;
						Bool_t take_jet_temp = kTRUE;
						for (Int_t jloop = 0; jloop < jets.size(); jloop++) 
						{
							take_jet_temp = kTRUE;
							if (jets[jloop].eta() < -1.5 || jets[jloop].eta() > 1.8) take_jet_temp = kFALSE;
							if (jet_en_corr_1stana(jets[jloop].eta(), jets[jloop].et(), period, mc_type) > 2.5) take_jet_temp = kFALSE;
							if (take_jet_temp) n_jets++;
						}
					...
					if (candidate_jet_number == -1 && ((check_cuts && !nodebugmode) || printOnlyCuts)) cout<< "###############################candidate_jet_number not found###############################"<<endl;
					...
					if (take_event && here_is_prph)
					{
						...
						{							
							...
							if (check_cuts && !nodebugmode) cout << "jet " << jloop << ": calling correction with parameters: " 
									<< jets[jloop].eta() << ", " << jets[jloop].et() << ", " << period << ", " << mc_type << endl
									<< "jet " << jloop << " of " << jets.size() 
									<< ", eta = " << jets[jloop].eta() 
							...
							{
								take_jet = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "jet with eta " << jets[jloop].eta() << " rejected because it is prompt photon jet " << endl;
							}
							if (et_jet_corr < ET_JET_CUT) 
							{
								take_jet = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "jet with eta " << jets[jloop].eta() << " rejected by et_jet_corr = "<< et_jet_corr << endl;
							}
							if (jets[jloop].eta() < -1.5 || jets[jloop].eta() > 1.8) 
							{
								take_jet = kFALSE;
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "jet with eta " << jets[jloop].eta() << " rejected by jets[" << jloop << "].eta() = "<< jets[jloop].eta() << endl;
							}
							// if (jloop == candidate_jet_number) take_jet = kFALSE;

							if (take_jet)
							{
								if ((check_cuts && !nodebugmode) || printOnlyCuts) cout << "jet selected " << jloop << ": " << jets[jloop].eta() << " " << jets[jloop].et() << endl;
								here_is_jet = kTRUE;
					...
					Double_t hardest_jet_et_corr = 0, hardest_jet_et = 0, hardest_jet_eta = -999., hardest_jet_phi = -999.;
					...
					if (inclusive_prph_analysis) here_is_jet = kTRUE;
					...
					if (take_event && here_is_jet && here_is_prph) 
					{
						...
						// x_gamma
							Double_t x_p = v_uncorr_prompt_photon->E() + v_uncorr_prompt_photon->Pz();
							x_p += jets[max_et_accomp_jet_candidate_number].e() + jets[max_et_accomp_jet_candidate_number].pz();
							x_p /= (2. * E_p);

							Double_t x_p_exp = v_uncorr_prompt_photon->Et() * TMath::Exp(  v_uncorr_prompt_photon->Eta());
							x_p_exp += jets[max_et_accomp_jet_candidate_number].et() * TMath::Exp(  jets[max_et_accomp_jet_candidate_number].eta());
							x_p_exp /= (2. * E_p);
							...
							if (!nodebugmode && (x_p >= 1 || x_p_exp >= 1)) cout << "x_p: " << x_p << " x_p_exp: " << x_p_exp <<endl;
							if (!nodebugmode) cout << "empz_jet = " << jets[max_et_accomp_jet_candidate_number].e() - jets[max_et_accomp_jet_candidate_number].pz()  << endl;
						...
						if (take_event_trig)
						{
							...
							/*
								...
								hist_mc_rad[0]->GetBinError(ii), "=", sqrt(hist_mc_rad[0]->GetSumw2()->GetAt(ii)), "!=", sqrt(hist_mc_rad[0]->GetBinContent(ii)), hist_mc_rad[0]->GetBinContent(ii)
							*/
							...
							*/
							...
							hist.det_cross_xp->Fill(x_p, wtx); 
							...
							hist.prof_det_cross_xp->Fill(x_p, x_p, wtx); 
							...
							det_cross_sec_bin_xp = hist.det_cross_xp->FindBin(x_p); 
							...
							for (Int_t i = 0; i < number_xp_bins; i++)
								if (x_p > xp_bin[i] && x_p < xp_bin[i+1]) 
								{
							...
							if (!nodebugmode)
							{
								...
								cout << selectedEventsCount+1 << " " << Runnr << " " << Eventnr << endl;
								cout << Siq2el[0] << "   " << Siecorr[0][2] * systElE << "   " << Sith[0]*180/TMath::Pi() << endl;
								cout << TMath::Abs(Sipos[0][0]) << "   " << Sipos[0][1] << "   " << Cal_empz << "   " << TMath::Abs(Zvtx) << endl;
								cout << max_et_candidate << " " << max_et_candidate_uncorr << "   " << v_corr_prompt_photon->Eta() 
									 << "   " << v_uncorr_prompt_photon->E()/v_prompt_photon_jet->E()
									 << "   " << v_uncorr_prompt_photon->Et()/v_prompt_photon_jet->Et() << "   " << v_corr_prompt_photon->E() << endl;
								cout << nVertTracks << "   " << cell_energy_frac
									 << "   " << 1 << "   " << glob_deltaz << endl;
								cout << glob_fmax << "   " << hardest_jet_eta << "   " 
									<< hardest_jet_et << "   " 
									<< hardest_jet_et_corr << 
							...
							hist.x_pomeron->Fill(x_p, wtx);
							hist.x_pomeron_unmarged->Fill(x_p, wtx);
							hist.x_pomeron_exp->Fill(x_p_exp, wtx);
							hist.x_pomeron_diff->Fill(x_p_exp - x_p);
							...
							for (Int_t sloop = 0; sloop < Sincand; sloop++)
							{
								...
								Double_t dr = TMath::Sqrt(pow(dphi, 2) + pow(deta, 2));
							...
							if (Sixel[0]*Siyel[0] != 0) hist.comp_s->Fill(Siq2el[0]/(Sixel[0] * Siyel[0]));
							...
							*/
							...
							hist.h2d_dphijet_dphigamma->Fill(hardest_jet_phi - Siph[0], delta_phi(v_corr_prompt_photon->Phi(), Siph[0]), wtx);
							...
							selectedEventsCount++;
							//printout interesting events
							if (print_interesting_events && et_3system_ev_ratio > 0.95 && (!nodebugmode)) 
							{
								cout << "\n============ INTERESTING EVENT ================" << endl;
			...
			// Hadron level selection
				...
				{
					sel_had_lev++;
					if (!nodebugmode) cout << "Selected on hadronLevel" << endl;
	...
	cout << "selected on detecter level: " << sel_det_lev << " ; selected on hadron level: " << sel_had_lev << endl;
	return kTRUE;
}
