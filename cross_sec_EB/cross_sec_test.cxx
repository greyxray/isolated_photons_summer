...
{  
    ...
    }
    {
        ...
        Test_Units test;
        test.Test_GetSumError();
        test.Test_GetFitRange( true);
    ...
    */

    {
            ...
            if (nodebugmode) cout << "AFTER  SCALING: mc QQ: " << hist_mc[0]->Integral() << " mc LL: " << hist_mc_rad[0]->Integral() << " mc bg: " << hist_mc_norad[0]->Integral() << " data: " << hist_data[0]->Integral()<<endl;
            ...
            //Стоп, разве это должно тут быть? разве это не сделано в hist.c? Да. Но Вообще говоря не играет роли, потому что входят в фотоны с параметром.
                 if (nodebugmode) dout("Error second scale:", (hist_data[0]->Integral() - hist_mc_rad[0]->Integral())/hist_mc[0]->Integral());
                 if (nodebugmode) dout("Error second scale:", hist_data[0]->Integral() / hist_mc_norad[0]->Integral());
            ...
            //Corrections
                // if(QQfit==1 && !whichCorrection.Contains("NoCorrection"))//Think how to extend it to other case
                // {
                //     if (whichCorrection.Contains("IanSg"))
                //         for (size_t j = 0; j != IanCorrectionSg.size(); ++j) 
                //         {
                //             hist_mc[0]->SetBinContent(j+1, hist_mc[0]->GetBinContent(j+1) * IanCorrectionSg[j]); 
                //             hist_mc[0]->SetBinError(j+1, hist_mc[0]->GetBinError(j+1) * IanCorrectionSg[j]);           
                //         }
                //     if (whichCorrection.Contains("PeterSg"))
                //         for (size_t j = 0; j != PeterCorrectionSg.size(); ++j) 
                //         {
                //             hist_mc[0]->SetBinContent(j+1, hist_mc[0]->GetBinContent(j+1) * PeterCorrectionSg[j]);  
                //             hist_mc[0]->SetBinError(j+1, hist_mc[0]->GetBinError(j+1) * PeterCorrectionSg[j]);                   
                //         }
                //     if (whichCorrection.Contains("PeterBg"))
                //         for (size_t j = 0; j != PeterCorrectionBg.size(); ++j) 
                //         {
                //             hist_mc_norad[0]->SetBinContent(j+1, hist_mc_norad[0]->GetBinContent(j+1) * PeterCorrectionBg[j]);  
                //             hist_mc_norad[0]->SetBinError(j+1, hist_mc_norad[0]->GetBinError(j+1) * PeterCorrectionBg[j]);                   
                //         }
                // }
            ...
            if (QQfit == 1 && fitWithLL==1 && fitWithLLinBg==0)
            {
             ...
             if (nodebugmode) dout("N +/- dN =", N, Nerr);
            
             for(Int_t j = 0;j<10;j++)//GetNbinsX()
                   if (nodebugmode) dout("csj=",j,")",hist_mc[0]->GetBinContent(j+1) );
            ...
            else if (QQfit == 0)
            {
             ...
             if (nodebugmode) dout("hist_res[0][3] = ", hist_res[0]->GetBinContent(3));
            ...
            // dout("hist_res[0][3] = ", hist_res[0]->GetBinContent(3));
                  if (nodebugmode) dout("hist_data[0][3] = ", hist_data[0]->GetBinContent(3));
            ...
            if (fitWithLLinBg == 1)
                ...
                str_temp2 += "_noLLinBg";
             if (nodebugmode) dout(tempstr + "_" + str_temp2  + ".png");
