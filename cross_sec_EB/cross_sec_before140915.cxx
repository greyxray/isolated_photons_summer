...
{
    if (f) {return h->FindBin(0.) + 1;} //left range
    else {return h->FindBin(0.8);}
}
  Hist hist;
  TH1D* hist_data[2];
  TH1D* hist_mc[2];
...
int main(int argc, char *argv[])
{
    ...
    {      
        ...
        chi2_et[i] = chi2(par, dof_et[i], left_et[i], right_et[i], "Barlow-Beeston") / 
                    dof_et[i];// достаем хи-квадрат dof = degries of freedom
        
        //chi squared per bin
        Double_t total_chi2 = 0;
        Int_t dof = 0;
        cout<<"Fit in bins of et: from bin "<< GetFitRange(hist_data[0], true)<<
        ...
        for(Int_t bin = GetFitRange(hist_data[0], true); bin < GetFitRange(hist_data[0], false); bin++) //? 02/07/15
        {
            ...
            total_chi2 += chi2_bin;
            dof++;
            cout << "bin " << bin << ", chi2 = " << chi2_bin << endl;
        ...
        c->Print(sc + ".eps");
        delete c;
        cout << "total chi2 = " << total_chi2 << endl;
        cout << "... per dof = " << total_chi2 / Double_t(dof-2) << endl;
        // ----------- end chi2 per bin
    ...
    {      
        ...
        hist_mc_photon[0] = (TH1D*)hist_mc_rad[0]->Clone(); hist_mc_photon[0]->SetName("photon");//? 020715  не так как в пред случае
...
//hist_mc_norad[0]->Scale((hist_data[0]->GetSum() - hist_mc_rad[0]->GetSum())/hist_mc_norad[0]->GetSum());
        ...
        //qq к ll+qq
        hist.h_deltaz_eta_prph_sum[i]->Scale((hist.h_deltaz_eta_data_sum[i]->GetSum() \
                                              - hist.h_deltaz_eta_rad_sum[i]->GetSum())\
                                             /hist.h_deltaz_eta_prph_sum[i]->GetSum());
        //ll к ll+qq
        hist.h_deltaz_eta_norad_sum[i]->Scale((hist.h_deltaz_eta_data_sum[i]->GetSum()\
                                               -hist.h_deltaz_eta_rad_sum[i]->GetSum())\
                                              /hist.h_deltaz_eta_norad_sum[i]->GetSum());
...
Double_t chi2(double* par, Int_t& ndf, Int_t& left, Int_t& right, TString s_method)
{
    ...
    if(fit_method == "deltaz") {
        ...
        cout<<"rightqwer "<<right<<" left "<< left<< endl; 
    }
    else
    if(fit_method == "fmax") {
        index = g_index_fmax;
