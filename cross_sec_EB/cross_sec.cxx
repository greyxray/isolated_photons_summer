...
//DoReparametrisation 221115
/*void DoReparametrisation221115(Int_t histindex, Double_t* param, Double_t* param_err, Int_t index)
{
    Double_t a = param[index], da = param_err[index];
    Double_t        d = hist.hist_data_sum[histindex]->Integral();
    Double_t       dd = TMath::Sqrt(hist.hist_data_sum[histindex]->Integral());
    Double_t        b = hist.hist_mc_norad_sum[histindex]->Integral();
    Double_t       db = TMath::Sqrt(hist.hist_mc_norad_sum[histindex]->Integral());
    Double_t        p = hist.hist_mc_photon_sum[histindex]->Integral();
    Double_t       dp = TMath::Sqrt(hist.hist_mc_photon_sum[histindex]->Integral());
    Double_t        l = hist.hist_mc_rad_sum[histindex]->Integral();
    Double_t       dl = TMath::Sqrt(hist.hist_mc_rad_sum[histindex]->Integral());
    Double_t        q = hist.hist_mc_sum[histindex]->Integral();
    Double_t       dq = TMath::Sqrt(hist.hist_mc_sum[histindex]->Integral());
     da = pow(d,-2)*\
...
//DoReparametrisation new
/*void DoReparametrisationVIAhistdata(Int_t histindex, Double_t* param, Double_t* param_err, Int_t index)
    {
        Double_t a = param[index], da = param_err[index];
        Double_t        d = hist_data[0]->Integral();
        Double_t       dd = TMath::Sqrt(d);
        Double_t        b = hist_mc_norad[0]->Integral();
        Double_t       db = TMath::Sqrt(db);
        Double_t        p = hist_mc_photon[0]->Integral();
        Double_t       dp = TMath::Sqrt(p);
        Double_t        l = hist_mc_rad[0]->Integral();
        Double_t       dl = TMath::Sqrt(l);
        Double_t        q = hist_mc[0]->Integral();
        Double_t       dq = TMath::Sqrt(q);
         da = pow(d,-2)*\
            TMath::Sqrt( pow(dd,2)*pow(l - a*p, 2) +\
                pow(d,2)*(pow(dl,2) + pow(a,2)*pow(dp, 2) + 
                    pow(da*p, 2))
...
/*!This function is usen only for photn*a+(1-a)bf fit. 
    The parameter will be the fraction of QQ' in data but not the 
    parameteret to build res curve by eq. res = qq'*a+ll+(1-a)bg.
    This function is called to the varianles like parameter_xgamma
    example:
...
// DoReparametrisationforQQ from controlplots 221115:
/*    void DoReparametrisationforQQVIAhistdata(Int_t histindex, Double_t* param, Double_t* param_err, Int_t index)
        {
            Double_t a = param[index], da = param_err[index];
            // Double_t        d = hist.hist_data_sum[histindex]->Integral();
            // Double_t       dd = TMath::Sqrt(hist.hist_data_sum[histindex]->Integral());
