...
int main(int argc, char *argv[])
{
  ...
  for(Int_t i=0; i<number_etbins; i++)
    {      
      ...
      cout << "PRPH scale factor in et bin " << i << ": " << (hist_data[0]->GetSum() - hist_mc_rad[0]->GetSum())/hist_mc[0]->GetSum() << " " 
      	   << (hist_data[0]->GetSum() - hist_mc_rad[0]->GetSum())/hist_mc[0]->GetSum()<< endl;
      cout << "first bin of prph before scaling: " << hist_mc[0]->GetBinContent(1) << endl;
      hist_mc[0]->Scale((hist_data[0]->GetSum() - hist_mc_rad[0]->GetSum())/hist_mc[0]->GetSum());
      cout << "first bin of prph after scaling: " << hist_mc[0]->GetBinContent(1) << endl;

      cout << "MCNC scale factor in et bin " << i << ": " << (hist_data[0]->GetSum() - hist_mc_rad[0]->GetSum())/hist_mc_norad[0]->GetSum() << " " 
      	   << (hist_data[0]->Integral() - hist_mc_rad[0]->Integral())/hist_mc_norad[0]->Integral()<< endl;
      cout << "Entries: data = " << hist_data[0]->GetEntries() << ", rad = " << hist_mc_rad[0]->GetEntries() << ", norad = " 
	   << hist_mc_norad[0]->GetEntries() << ", prph = " << hist_mc[0]->GetEntries() << endl;
      cout << "GetSum: data = " << hist_data[0]->GetSum() << ", rad = " << hist_mc_rad[0]->GetSum() << ", norad = " 
	   << hist_mc_norad[0]->GetSum() << ", prph = " << hist_mc[0]->GetSum() << endl;
      cout << "Integral: data = " << hist_data[0]->Integral() << ", rad = " << hist_mc_rad[0]->Integral() << ", norad = " 
	   << hist_mc_norad[0]->Integral() << ", prph = " << hist_mc[0]->Integral() << endl;
      ...
      hist.h_deltaz_et_norad_sum[i]->Scale((hist.h_deltaz_et_data_sum[i]->GetSum()-hist.h_deltaz_et_rad_sum[i]->GetSum())/hist.h_deltaz_et_norad_sum[i]->GetSum());
      ...
      left_et[i]++;
      hist.h_deltaz_et_norad_sum[i]->Scale(1-param_et[i]);
      hist.h_deltaz_et_prph_sum[i]->Scale(param_et[i]);
      hist.h_deltaz_et_res[i]->Add(hist.h_deltaz_et_norad_sum[i]);
      hist.h_deltaz_et_res[i]->Add(hist.h_deltaz_et_prph_sum[i]);
      cout << "par in bin et " << i << ": " << param_et[i] << " +- " << param_err_et[i] << endl;
    }
  ...
  for(Int_t i=0; i<number_etabins; i++)
    {      
      ...
      cout << "par in bin eta " << i << ": " << param_eta[i] << " +- " << param_err_eta[i] << endl;
    }
  ...
  for(Int_t i=0; i<number_xbins; i++)
    {      
      ...
      cout << "par in bin x " << i << ": " << param_x[i] << " +- " << param_err_x[i] << endl;
    }
  ...
  for(Int_t i=0; i<number_et_jetbins; i++)
    {      
      ...
      cout << "par in bin et_jet " << i << ": " << param_et_jet[i] << " +- " << param_err_et_jet[i] << endl;
    }
  ...
  for(Int_t i=0; i<number_eta_jetbins; i++)
    {      
      ...
      cout << "par in bin eta_jet " << i << ": " << param_eta_jet[i] << " +- " << param_err_eta_jet[i] << endl;
    }
...
Double_t chi2(double* par, Int_t& ndf, Int_t& left, Int_t& right, TString s_method)
{
  ...
  if(fit_method == "deltaz") {
    ...
    right = hist_data[index]->FindBin(0.6)+5;//20;//hist_data[index]->FindBin(0.8);//18;//hist_data[index]->FindBin(0.8); //0.8 / 0.05
  }
  else
    if(fit_method == "fmax") {
      index = g_index_fmax;
   ...
   for(Int_t i=left/*hist_data_sum[index]->FindBin(left)-1*/; i<right/*hist_data_sum[index]->FindBin(right)*//*GetNbinsX()*/; i++)
	...
	cout << "dz " << i << "\t" << "numer=" << diff*diff << "\tdenom=" << denom << "\tndf=" << ndf << endl;
	cout <<i+1 << " yhDat=" << v_d << "\tyh1= " << v_q << "\tyh2= " << v_bg  << "\tyhDatE=" << s_d << "\tyh1E= " << s_q << "\tyh2E= " << s_bg << endl;
	// ====================================================================================================
	//
	// Ian
	//
