...
unsigned int NoScalingForBinsTest = 1;
...
int main(int argc, char *argv[])
{
...

    {
            ...
            hist_mc_photon[0] = (TH1D*)hist.hist_mc_rad_sum[i]->Clone();             hist_mc_photon[0]->SetName("photon");
            hist_mc_photon[0]->Add(hist_mc[0], QQfit + (1 - QQfit) * hist.total_luminosity_data/hist.lumi_mc_prph);//LL + QQ' Add(hist_mc[0], hist.total_luminosity_data/hist.lumi_mc_prph); - decided on 231115
            hist_mc_photon[0]->Scale(hist_data[0]->Integral()/hist_mc_photon[0]->Integral());
            ...
            hist_res[0] = (TH1D*)hist.hist_mc_rad_sum[i]->Clone(); 
           

           for(Int_t j = 0;j<hist_mc_photon[0]->GetNbinsX();j++)
            {
                dout("j=",j,")",hist_mc_photon[0]->GetBinContent(j+1) );//
            }
            ...
            DoComplicatedScale(hist_res[0], param[0], param_err[0]);
            ...
            hist_data[0]->Sumw2();
                ...
                TString s2; s2.Form("#chi^{2} / %i-2 = %.2f", ndf+2, chi_squared);
...

            {
                    ...
                    line035->DrawLine(0.35, 0., 0.35, h_window_control->GetYaxis()->GetXmax());
            }
            c->Print(tempstr + ".png");
