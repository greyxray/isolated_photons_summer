...
/*, Double_t integral_data, Double_t integral_ll, Double_t integral_data_err, Double_t integral_ll_err*/) 
{
    ...
    for(Int_t i = 0; i < LLplusQQ->GetNbinsX(); i++) 
    {
        ...
        if (name.Contains("deta_e_ph"))
         {
          if (nodebugmode) cout<< "After" << i<<") " <<"LLplusQQ = "<<LLplusQQ->GetBinContent(i+1) <<" +/- "<< LLplusQQ->GetBinError(i+1)<<endl;
         }
    }
    ...
    for(Int_t i = 0; i < nbins; i++)
    {
        ...
        */
        ...
        if(!nodebugmode && name.Contains("xgamma")) cout<< name << " cros sec in bin " << i << ": " << cross_sec 
