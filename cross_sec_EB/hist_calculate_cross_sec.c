...
/*, Double_t integral_data, Double_t integral_ll, Double_t integral_data_err, Double_t integral_ll_err*/) 
{
    ...
    Double_t integral_data, integral_LLplusQQ, integral_data_err, integral_LLplusQQ_err,
    ...
    TH1D* LLplusQQ = (TH1D*)ll_det->Clone(); //h_det_rad_sum LL det
    ...
    //?
    //scale LLplusQQ by ndata/nllplusqq
    for(Int_t i = 0; i < LLplusQQ->GetNbinsX(); i++) /*какой смысл делать побыновый скейл?*/
    {
        ...
        cout << "bin " << i << ", data = " << integral_data << " +- " << integral_data_err
             << ", ll+qq = " << integral_LLplusQQ << " +- " << integral_LLplusQQ_err << endl;
        
        scale_data = integral_data / integral_LLplusQQ;// data/(QQ+LL)  
        scale_err1 = integral_data_err / integral_LLplusQQ;// data_err/QQLL
        scale_err2 = integral_data * integral_LLplusQQ_err / 
                    (integral_LLplusQQ * integral_LLplusQQ);// data*QQLL_err/LLQQ^2
        scale_data_err = TMath::Sqrt(scale_err1*scale_err1 + scale_err2*scale_err2);
        
        cout << "scale_data_ll (in bin "<< i<< ") = " << data->GetBinContent(i+1) << " - "
             << ll_det->GetBinContent(i+1) << " = " << scale_data << " +- " << scale_data_err << endl;
        ...
        Double_t new_err = TMath::Sqrt(cont * cont * scale_data_err * scale_data_err
                         + err * err * scale_data * scale_data),
    ...
    for(Int_t i = 0; i < nbins; i++)
    {
        ...
        Double_t low_edge = det->GetBinLowEdge(i+1),
        ...
        Double_t err1 = 0., err2 = 0., err3 = 0., err11 = 0., err22 = 0., err = 0.; //statistic, acceptance, luminosity, full
        ...
        // new and cool
        //*/
        Double_t C_acc = acc->GetBinContent(i+1),
        ...
        Double_t cross_sec = ((ll_plus_qq - ll_events)/C_acc + ll_events/C_ll_acc)
                             / (bin_width * Lumi);
        Double_t prph_cross_sec = (ll_plus_qq - ll_events) / (C_acc * bin_width * Lumi);
        Double_t ll_cross_sec = ll_events/(C_ll_acc * bin_width * Lumi);
        Double_t prph_mc_cross_sec = prph_had / (bin_width * 3552.40);
        Double_t ll_mc_cross_sec = ll_had_events / (bin_width * (lumi_mc_bg[0] + lumi_mc_bg[1] + lumi_mc_bg[2]));//? 160715
        ...
        cout << "prph data: " << prph_cross_sec << ", prph mc: " << prph_mc_cross_sec << ", prph_had = " << prph_had <<  endl;
        cout << "ll data: " << ll_cross_sec << ", ll mc: " << ll_mc_cross_sec << endl;
        fout << "h_1st[0]->SetBinContent(" << i+1 << ", " << cross_sec << ");" << endl;
        fout << "h_1st[0]->SetBinError(" << i+1 << ", " << err << ");" << endl;
    }//for i over bins
    fout << endl;
    cout << "sigma_tot = " << sigma_tot << " +- " << TMath::Sqrt(sigma_tot_err) << " pb" << endl;
}
